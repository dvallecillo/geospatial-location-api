    ...
    public class QueryHandlerTests
    {
                ...
                GetLocationsQueryHandler sut)
        {
            ...
            locationQueries.Verify(
                x => x.GetClusterCentersAsync(), Times.Never);

            locationQueries.Verify(
                x => x.GetClusterAsync(It.IsAny<long>()), Times.Never);
        }
                ...
                GetLocationsQueryHandler sut)
        {
            ...
            locationQueries.Setup(
                    x => x.GetClusterCentersAsync())
                .Returns(Task.FromResult(clusters.Select(c => new KeyValuePair<long, Point>(c.Id, c.Center))));

            locationQueries.Setup(
                    x => x.GetClusterAsync(It.IsAny<long>()))
                .Returns(Task.FromResult(clusters.First()));
            ...
            locationQueries.Verify(
                x => x.GetClusterCentersAsync(), Times.Once);
            locationQueries.Verify(
                x => x.GetClusterAsync(It.IsAny<long>()), Times.Exactly(clusters.Count));
        }
